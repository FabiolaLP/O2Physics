  ...
  void process(aod::McCollisions const& cols, aod::McParticles const& particles, TracksWCovMc const& tracks)
  {
    ...
    for (auto const& col : cols) {
      ...
      for (auto& particle : particles) {
        ...
        if (particle.mcCollisionId() != colId)
          continue;
        ...
        if (particle.pdgCode() == -2112 && particle.isPhysicalPrimary()) {
          ...
          if (TMath::Abs(particle.eta()) < 1.5 && TMath::Abs(particle.vz()) < 5.3)
        ...
        if (particle.pdgCode() == -2212 && particle.isPhysicalPrimary()) {
          ...
          if (TMath::Abs(particle.eta()) < 1.5 && TMath::Abs(particle.vz()) < 5.3)
            histos.fill(HIST("antip_p_ITScuts"), particle.p());
        }
        // Primary protons
        if (particle.pdgCode() == 2212 && particle.isPhysicalPrimary()) {
          ...
          if (TMath::Abs(particle.eta()) < 1.5 && TMath::Abs(particle.vz()) < 5.3)
            histos.fill(HIST("p_p_ITScuts"), particle.p());
        }
        ...
        if (particle.pdgCode() == -2212 && isSecondaryFromMaterial && !particle.mothersIds().empty()) {
          for (auto mother : particle.mothers_as<aod::McParticles>()) {
            if ((mother.isPhysicalPrimary()))
              histos.fill(HIST("primmom_test"), particle.p());
          }
        }
              ...
              if (TMath::Abs(mother_eta) < 1.5 && TMath::Abs(mother_vz) < 5.3) {
                ...
                for (auto& particle1 : particles) {
                  ...
                  if (particle1.mcCollisionId() != colId)
                    continue;
                  if (particle1.pdgCode() != -211 || !isSecondaryFromMaterial1 || particle1.mothersIds().empty())
                    continue;
                ...
                for (auto& particle2 : particles) {
                  ...
                  if (particle2.pdgCode() != 211 || !isSecondaryFromMaterial2 || particle2.mothersIds().empty())
                    continue;
                ...
                if (pion_p == false && pion_m == false) {
                  ...
                  for (auto& particle3 : particles) {
                    ...
                    if (particle3.pdgCode() != 2212 || !isSecondaryFromMaterial3 || particle3.mothersIds().empty())
                      continue;
                    ...
                    if (p_vx == antip_vx && p_vy == antip_vy && p_vz == antip_vz) {
                      ...
                      if (k_plane == k_e && k_plane == k_p && k_plane >= 0) {
                        ...
                        if (mother_pdg == -2112) {
                          ...
                          if (TMath::Abs(mother_eta) < 0.9 && TMath::Abs(mother_vz) < 5.3)
                            histos.fill(HIST("cex_pairmc_pITScuts"), cex_pairmc_p);
                          // CEX pair normalized
                          if (mother_p != 0)
                            histos.fill(HIST("cexn_pairmc_p"), cex_pairmc_p / mother_p);
                          if (mother_pt != 0)
                            histos.fill(HIST("cexn_pairmc_pt"), cex_pairmc_pt / mother_pt);
                          if (mother_pz != 0)
                            histos.fill(HIST("cexn_pairmc_pz"), cex_pairmc_pz / mother_pz);
                        }
                        // BG mother
                        if (mother_pdg != -2112) {
                          // CEX pair
                          ...
                          if (TMath::Abs(mother_eta) < 0.9 && TMath::Abs(mother_vz) < 5.3)
                            histos.fill(HIST("cexbg_pairmc_pITScuts"), cex_pairmc_p);
                        }
                        ...
                        if (p_hasTrack == true && antip_hasTrack == true) {
                          ...
                          if (!(pRow && apRow)) {
                            ...
                            if (nCand > 0 && (status == DCAFitter2::FitStatus::Converged || status == DCAFitter2::FitStatus::MaxIter)) {
                              ...
                              if (mother_pdg == -2112)
                                histos.fill(HIST("cex_pairtrk_vtxfit_dcaPair"), dcaPair);
                              if (mother_pdg != -2112)
                                histos.fill(HIST("cexbg_pairtrk_vtxfit_dcaPair"), dcaPair);
